...
<div class="w-full max-w-5xl">
    ...
    <form method="POST" action="{{ route('choose-kingdom.store') }}" id="kingdom-form">
        @csrf

        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            @foreach($kingdoms as $kingdom)
                <label data-kingdom-card
                    ...
                    <input type="radio" name="kingdom_id" value="{{ $kingdom->id }}" class="sr-only" {{ old('kingdom_id') == $kingdom->id ? 'checked' : '' }}>

                    {{-- Color gradient header --}}
                    <div class="relative h-32 overflow-hidden">
                        <div class="h-full w-full transition-transform duration-500 group-hover:scale-105"
                             style="background: linear-gradient(135deg, {{ $kingdom->color }}30, {{ $kingdom->color }}08);"></div>
                        ...
                        <div data-kingdom-check class="absolute right-5 top-5 hidden flex h-9 w-9 items-center justify-center rounded-full bg-gold">
                    ...
                    <div class="p-5">
                        <h2 class="font-display mb-1 text-xl" style="color: {{ $kingdom->color }}">{{ $kingdom->name }}</h2>
                        <p class="mb-3 text-xs text-text-muted">{{ $kingdom->description }}</p>

                        <div class="space-y-1 border-t border-border pt-3">
                            <div class="text-[10px] text-text-subtle">CITIES:</div>
                            @foreach($kingdom->cities as $city)
                                <div class="text-xs text-text">• {{ $city->name }}</div>
                            @endforeach
                        </div>
                    </div>
                </label>
        ...
        <div class="mt-8 flex justify-center">
            <button type="button" id="kingdom-confirm-btn"
                    class="btn-primary px-10 py-3 text-base opacity-50"
                    disabled
                    onclick="document.getElementById('kingdom-form').submit()">
...
<script>
document.querySelectorAll('[data-kingdom-card]').forEach(card => {
    card.addEventListener('click', () => {
        // Reset all cards
        document.querySelectorAll('[data-kingdom-card]').forEach(c => {
            c.classList.remove('border-gold', 'shadow-[0_0_30px_rgba(212,175,55,0.3)]');
            c.classList.add('border-border');
            c.querySelector('[data-kingdom-check]').classList.add('hidden');
        });
        // Highlight selected
        card.classList.remove('border-border');
        card.classList.add('border-gold', 'shadow-[0_0_30px_rgba(212,175,55,0.3)]');
        card.querySelector('[data-kingdom-check]').classList.remove('hidden');
        // Enable button
        const btn = document.getElementById('kingdom-confirm-btn');
        btn.disabled = false;
